...
public class ColorSerialization
{
    public static byte[] SerializeColor(object target)
    {
        ...
        Array.Copy(BitConverter.GetBytes(color.r), 0, result, 0, 4);
        Array.Copy(BitConverter.GetBytes(color.g), 0, result, 4, 4);
        Array.Copy(BitConverter.GetBytes(color.b), 0, result, 8, 4);
        Array.Copy(BitConverter.GetBytes(color.a), 0, result, 12, 4);
